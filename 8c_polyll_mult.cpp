...
PolyNode* poly_add_alt(PolyNode *p1, PolyNode *p2) {
	PolyNode* ret = NULL;
	while(p1 != NULL) {
		poly_insert_into(&ret, poly_create(p1->coeff, p1->exp));
		p1 = p1->next;
	}
	while(p2 != NULL) {
		poly_insert_into(&ret, poly_create(p2->coeff, p2->exp));
		p2 = p2->next;
	}
	return ret;
}

PolyNode* poly_mult(PolyNode *p1, PolyNode *p2) {
	PolyNode* ret = NULL;
	while(p1 != NULL) {
		PolyNode *temp = p2;
		while(temp != NULL) {
...
int main() {
	PolyNode* p1 = poly_input(false, "poly1.txt");
	PolyNode* p2 = poly_input(false, "poly2.txt");
	PolyNode *sum = poly_mult(p1, p2);
